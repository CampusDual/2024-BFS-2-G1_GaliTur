...
export class PackEditComponent {
  ...
  constructor(private ontimizeManagePackService: OntimizeService,private ontimizeManagePackImageService: OntimizeService,
  ...
  consultarDatosPackImage(id:number){
    this.ontimizeManagePackImageService.query({pck_id:id},["img_id"],"imagePack").subscribe((response) => 
      {
  ...
  protected configurePackImageService() {
    const conf = this.ontimizeManagePackImageService.getDefaultServiceConfiguration("imagePacks");
